...
public static class MovementPhysicsController
{
    ...
    private static void WalkOnGround(PlayerController player, Vector3 input)
    {
        ...
        if (player.groundCollider.TryGetComponent<Rigidbody>(out Rigidbody groundbody) && player.groundCollider.CompareTag("Walkable Moving Surface")) player.transform.parent = groundbody.transform;
    ...
    {
        if (!player || player.jumpCooldown || (!player.isGrounded && !player.doubleJumpReady)) return;

        PlayerAnimatorController.UpdatePlayerAnim(PlayerAnimBooleanStates.IsGrounded, player.isGrounded, player.animator);
        PlayerAnimatorController.UpdatePlayerAnim(PlayerAnimTriggerStates.Jump, player.animator);

        if (player.grappleHookObj) { GrapplingPhysicsController.GrappleLunge(player); return; }
        if (player.isGrounded) player.StartCoroutine(StartJumpForce());
        else if (player.doubleJumpReady) player.StartCoroutine(StartDoubleJumpForce());
        ...
        IEnumerator StartDoubleJumpForce()
        {
            ...
            Vector3 newHorizontalVelocity = Vector3.Lerp(currentHorizontalVelocity, jumpVector, 0.55f);
    ...
    public static void Stomp(PlayerController player)
    {
        if (!player || !player.body || player.isGrounded) return;
        if (!player.jumpCooldown) player.body.velocity = new Vector3(0, player.body.velocity.y > -player.movementPhyObj.stompSpeed ? -player.movementPhyObj.stompSpeed : player.body.velocity.y, 0);
        if (!player.grappleJoint) PlayerAnimatorController.UpdatePlayerAnim(PlayerAnimTriggerStates.Stomp, player.animator);
    }
